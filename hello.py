...
def scan_output(i):
    sys.stdout.flush()
    sys.stdout.write('\r[*] Đã scan chừng này: {0} | ' \
            'Có thể lượm: {1}'.format(i, len(HOSTLIST)))
...
def crack(target, port, user, passwd, outfile, timeo, i):
    ...
    try:
        ...
        output = 'Hên xui! maybe a honeypot or false positive'
        login = '%s|%s|%s' \
    ...
    try:
        for user in users:
            #for password in passwords:
                dkmuser,dkmpass=user.split("|")
                Run = threading.Thread(target=crack, args=(target, port, dkmuser,
                    dkmpass, outfile, timeout, 0,))
                Run.start()
...
def fork_it(args):
    ...
    print '[*] Sẽ lượm %d cháu\n' \
            ...
            '[*] Số threads mỗi host: %d' % (len_hosts, childs, threads)

    i = 1
    for host in HOSTLIST:
        host = host.replace('\n', '')
        print '[*] Đang lượm %s [%d/%d]' % (host, i, len_hosts)
...
# output when qespresso.py finished all routines
def finished():
    print '[*] Hết phim!'
